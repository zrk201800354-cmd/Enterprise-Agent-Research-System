...
def sp500_symbols() -> list[str]:
    return [
        ...
        "FAST", "FBHS", "FCX", "FDS", "FDX", "FE", "FFIV", "FIS", "FISV", "FLT",
...
def popular_tech_symbols() -> list[str]:
    return [
        "PLTR", "SNDK", "ARM", "SMCI", "SNOW", "CRWD", "ZS", "PANW", "NET",
        "DDOG", "MDB", "BILL", "HUBS", "VEEV", "OKTA", "SPLK", "TWLO", "SQ",
        "SHOP", "ROKU", "U", "RBLX", "COIN", "HOOD", "SOFI", "AFRM", "UPST",
        "AI", "SYM", "IONQ", "RGTI", "QUBT", "LMND", "OPEN", "WISH", "CLOV",
        "TLRY", "SNDL", "MARA", "RIOT", "CLSK", "BITF", "HUT", "BTBT",
    ]


def get_full_universe() -> list[str]:
    sp = sp500_symbols()
    tech = popular_tech_symbols()
    seen: set[str] = set()
    combined: list[str] = []
    for s in sp + tech:
        if s not in seen:
